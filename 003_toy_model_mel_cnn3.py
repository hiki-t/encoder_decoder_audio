...
class AudioCNN(nn.Module):
    def __init__(self, num_classes=10, input_channels=1, n_mels=128, max_time=1501, dropout=0.3):
        super(AudioCNN, self).__init__()
        # Input: (batch, 1, n_mels, time) e.g., (batch, 1, 128, 1501)
        self.conv2d_1 = nn.Conv2d(input_channels, 16, kernel_size=3, stride=1, padding=1)
        self.bn2d_1 = nn.BatchNorm2d(16) # For each channel in the batch, it subtracts the mean and divides by the standard deviation
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        # x: (batch, 1, n_mels, time) # torch.Size([32, 1, 128, 1501])
        x = self.conv2d_1(x) # torch.Size([32, 16, 128, 1501])
        x = self.bn2d_1(x)
        x = self.relu(x)
        x = self.pool2d_1(x) # torch.Size([32, 16, 64, 750])
        x = self.dropout(x)  # Dropout after first 2D block

        x = self.conv2d_2(x) # torch.Size([32, 32, 64, 750])
        x = self.bn2d_2(x)
        x = self.relu(x)
        x = self.pool2d_2(x) # torch.Size([32, 32, 32, 375])
