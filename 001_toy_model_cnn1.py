...
class AudioCNN(nn.Module):
    def __init__(self, num_classes=10, input_channels=1, time_dim=14_004, image_width=100):
        ...
        self.conv2d_1 = nn.Conv2d(16, 16, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.pool1d(x)  # (batch, 16, timesteps//4)
        ...
        image_height = timesteps // self.image_width
        x = x[:, :, :image_height * self.image_width]  # trim if not divisible
        x = x.reshape(batch, channels, image_height, self.image_width)
